...
- **Expanded Diagnostics** for malformed endpoint URLs, URL credentials/query/hash/path quality issues, invalid headers JSON, empty header names, duplicate header keys (case-insensitive), missing content negotiation headers, auth scheme warnings, and header whitespace validation.
...
- **Diagnostics Hygiene Pack** adds query-style checks (tabs/trailing whitespace/long lines/TODO comments/mixed indentation), endpoint hygiene checks (localhost/default ports/root/trailing/versioned paths + protocol validation), and header safety checks (wildcard Accept, non-JSON content type, empty Bearer/API key, oversized/control-character header values, Cookie/Host overrides).
