    ...
    operations: [
      {
        ...
        name: "GetUsers",
        variables: [],
        fields: [{ name: "users", args: [{ name: "first", value: 5 }], fields: [] }],
      },
      {
        type: "mutation",
        name: "UpdateUser",
        ...
        fields: [
          {
            name: "post",
            args: [{ name: "id", value: "$id" }],
            fields: [{ name: "title" }, { name: "author", fields: [{ name: "name" }] }],
          },
        ],
      },
    ],
...
test("stores history entry for missing endpoint", async () => {
  ...
  assert.equal(state.history[0].error, "Endpoint is required before executing a query.")
...
test("removes and clears unpinned history entries", () => {
  ...
  graphqlStore.importHistory([{ id: "entry-3", query: "query { three }", status: "success" }])
...
test("importHistory deduplicates by id and respects limit", () => {
  ...
  graphqlStore.importHistory([{ id: "entry-10", query: "query { updated }", status: "success" }])

  const state = graphqlStore.getState()
  assert.equal(state.history.length, 50)
  assert.equal(state.history.find((entry) => entry.id === "entry-10").query, "query { updated }")
})
